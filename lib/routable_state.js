(function() {
  ...
  // subclasses State() for RoutableState() // constructor, composition, prototypes, constructor
  ...
  RoutableState.prototype.route = function(pattern, state, opts) {
    ...
    // smart parameters and remove state
    if (arguments.length === 2 && typeof state !== 'string' && !(state instanceof State)) {
      opts = state;
      state = null;
    }

    // find _state's <State> representation
    _state = state || '.';
    ...
    // invoke target {_state}'s #_route(), which calls router.define(pattern) and registers Router
    //    {enterHandler} and {exitHandler}
  ...
  RoutableState.prototype._route = function(pattern, opts) {
    // only one route may be on each state
    if (this.__route__) {
      throw new Error("RoutableState#route: a route has already been defined on " + this);
    }

    // if state is attached, calls register route
    if (this.isAttached()) {
  ...
  RoutableState.prototype._registerRoute = function(pattern, opts) {
    ...
    // defines an enter handler that sets {__route__} and {__params__} for {thisState}
    this.enter(function(ctx) {
      router.route(this.__route__);
      router.params(util.pick(ctx || {}, this.__route__.names));
    });

    // defines exitHandlers that unsets __route__.names
  ...
  // NOTE: #start, #stop, #params, #unknown all defers to router
  ...
  RoutableState.prototype.urlFor = function(path, params) {
    ...
    // tries to call {this.resolve(path)} to get a <State>

    // throws if state wasn't resolved
    if (!state) {
      throw new Error('RoutableState#urlFor: could not resolve path `' + path + '`');
    }

    // throws if state doesn't have a route
    if (!state.__route__) {
      throw new Error('RoutableState#urlFor: state `' + path + '` does not have a route defined');
    }

    // defers logic to Router#urlFor -> buildUrl -> concat(generatePath, generateSearch)
